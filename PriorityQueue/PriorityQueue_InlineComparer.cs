...
{
    public class PriorityQueue_InlineComparer<TElement, TPriority>
    {
        ...
        #region Constructors
        public PriorityQueue_InlineComparer() : this(0, null)
        {

        }

        public PriorityQueue_InlineComparer(int initialCapacity) : this(initialCapacity, null)
        {

        }

        public PriorityQueue_InlineComparer(IComparer<TPriority>? comparer) : this(0, comparer)
        {

        }

        public PriorityQueue_InlineComparer(int initialCapacity, IComparer<TPriority>? comparer)
        {
        ...
        public PriorityQueue_InlineComparer(IEnumerable<(TElement Element, TPriority Priority)> values) : this(values, null)
        {

        }

        public PriorityQueue_InlineComparer(IEnumerable<(TElement Element, TPriority Priority)> values, IComparer<TPriority>? comparer)
        {
            _priorityComparer = comparer;
            _heap = Array.Empty<HeapEntry>();
            _count = 0;
        ...
        public TElement EnqueueDequeue(TElement element, TPriority priority)
        {
            ...
            if (_priorityComparer is IComparer<TPriority> comparer ? 
                comparer.Compare(priority, minEntry.Priority) <= 0 :
                Comparer<TPriority>.Default.Compare(priority, minEntry.Priority) <= 0)
            {
                return element;
            }
        ...
        {
            private readonly PriorityQueue_InlineComparer<TElement, TPriority> _priorityQueue;

            internal UnorderedItemsCollection(PriorityQueue_InlineComparer<TElement, TPriority> priorityQueue)
            {
            ...
            {
                private readonly PriorityQueue_InlineComparer<TElement, TPriority> _queue;
                private readonly int _version;
                private int _index;
                private (TElement Element, TPriority Priority) _current;

                internal Enumerator(PriorityQueue_InlineComparer<TElement, TPriority> queue)
                {
                ...
                {
                    PriorityQueue_InlineComparer<TElement, TPriority> queue = _queue;
        ...
        private void SiftUp(int index, in TElement element, in TPriority priority)
        {
            if (_priorityComparer is IComparer<TPriority> comparer)
            {
                SiftUpWithComparer(index, in element, in priority, comparer);
            }
            else
            {
                SiftUpDefaultComparer(index, in element, in priority);
            }
        }

        private void SiftUpWithComparer(int index, in TElement element, in TPriority priority, IComparer<TPriority> comparer)
        {
            while (index > 0)
            {
        ...
        private void SiftUpDefaultComparer(int index, in TElement element, in TPriority priority)
        {
            while (index > 0)
            {
                int parentIndex = (index - 1) >> 2;
                ...
                if (Comparer<TPriority>.Default.Compare(parent.Priority, priority) <= 0)
                {
                    // parentPriority <= priority, heap property is satisfed
                    break;
                }
        ...
        private void SiftDown(int index, in TElement element, in TPriority priority)
        {
            if (_priorityComparer is IComparer<TPriority> comparer)
            {
                SiftDownWithComparer(index, in element, in priority, comparer);
            }
            else
            {
                SiftDownDefaultComparer(index, in element, in priority);
            }
        }

        private void SiftDownWithComparer(int index, in TElement element, in TPriority priority, IComparer<TPriority> comparer)
        {
            int minChildIndex;
            int count = _count;
            HeapEntry[] heap = _heap;
        ...
        private void SiftDownDefaultComparer(int index, in TElement element, in TPriority priority)
        {
            int minChildIndex;
            int count = _count;
            HeapEntry[] heap = _heap;
            ...
            while ((minChildIndex = (index << 2) + 1) < count)
            {
                ...
                // compare with inserted priority
                if (Comparer<TPriority>.Default.Compare(priority, minChild.Priority) <= 0)
                {
                    // priority <= minChild, heap property is satisfied
                    break;
                }
