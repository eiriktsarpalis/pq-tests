...
{
    public class PriorityQueue_Comparable<TElement, TPriority> 
        where TPriority : notnull, IComparable<TPriority>
    {
        ...
        #region Constructors
        public PriorityQueue_Comparable() : this(0)
        {

        }

        public PriorityQueue_Comparable(int initialCapacity)
        {
        ...
        public PriorityQueue_Comparable(IEnumerable<(TElement Element, TPriority Priority)> values)
        {
            _heap = Array.Empty<HeapEntry>();
            _count = 0;
        ...
        public TElement EnqueueDequeue(TElement element, TPriority priority)
        {
            ...
            if (priority.CompareTo(minEntry.Priority) <= 0)
            {
                return element;
            }
        ...
        {
            private readonly PriorityQueue_Comparable<TElement, TPriority> _priorityQueue;

            internal UnorderedItemsCollection(PriorityQueue_Comparable<TElement, TPriority> priorityQueue)
            {
            ...
            {
                private readonly PriorityQueue_Comparable<TElement, TPriority> _queue;
                private readonly int _version;
                private int _index;
                private (TElement Element, TPriority Priority) _current;

                internal Enumerator(PriorityQueue_Comparable<TElement, TPriority> queue)
                {
                ...
                {
                    PriorityQueue_Comparable<TElement, TPriority> queue = _queue;
        ...
        private void AppendRaw(IEnumerable<(TElement Element, TPriority Priority)> values)
        {
            ...
            // TODO: cleanly detect null priorities
        ...
        private void SiftUp(int index, in TElement element, in TPriority priority)
        {
            while (index > 0)
            {
                ...
                if (parent.Priority.CompareTo(priority) <= 0)
                {
        ...
        private void SiftDown(int index, in TElement element, in TPriority priority)
        {
            ...
            while ((minChildIndex = (index << 2) + 1) < count)
            {
                ...
                for (int nextChildIndex = minChildIndex + 1; nextChildIndex < childUpperBound; nextChildIndex++)
                {
                    ...
                    if (nextChild.Priority.CompareTo(minChild.Priority) < 0)
                    {
                ...
                if (priority.CompareTo(minChild.Priority) <= 0)
                {
